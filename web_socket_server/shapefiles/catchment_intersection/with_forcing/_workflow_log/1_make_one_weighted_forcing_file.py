...
if forcing_easymore_path == 'default':
    forcing_easymore_path = make_default_path('forcing/3_temp_candex') # outputs a Path()
else:
...
esmr.temp_dir = str(forcing_easymore_path) + '/' # Path() to string; ensure the trailing '/' CANDEX wants

# Output folder where the catchment-averaged forcing will be saved
esmr.output_dir = str(forcing_basin_path) + '/' # Path() to string; ensure the trailing '/' CANDEX wants
